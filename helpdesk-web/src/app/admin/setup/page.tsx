  ...
  useEffect(() => {
    ...
    const unsubscribeUser = onSnapshot(
      ...
      (error) => {
        ...
        console.error("Admin setup user listener error:", error);
      },
    );
    ...
    <main className="relative min-h-screen overflow-hidden bg-[radial-gradient(circle_at_15%_10%,_rgba(34,211,238,0.22),_transparent_30%),radial-gradient(circle_at_85%_85%,_rgba(14,165,233,0.2),_transparent_34%),linear-gradient(180deg,_#040b15_0%,_#071223_44%,_#0e1a30_100%)] px-4 py-6 text-slate-100 sm:px-6 lg:px-10">
      ...
      <section className="mx-auto w-full max-w-2xl px-4 sm:px-6">
        ...
        <div className="rounded-xl border border-slate-700/40 bg-slate-900/40 p-8 shadow-2xl shadow-slate-950/60 backdrop-blur-xl">
          ...
          <p className="text-slate-400 mb-8">Configure your admin access</p>
          ...
          <div className="space-y-6">
            ...
            {/* Admin Status Check */}
            <div className="border-t border-slate-700/30 pt-6">
              <p className="text-xs uppercase tracking-wider text-slate-400 mb-2">Admin Status</p>
              {!loading && (
                <p className="text-slate-300">
                  {adminExists
                    ? "❌ An admin already exists in the system"
                    : "✓ No admin found - You can become the first admin"}
                </p>
              )}
            </div>
            ...
            {message && (
              ...
              >
                <p className={setupComplete ? "text-green-200" : "text-rose-200"}>{message}</p>
              </div>
            )}
            ...
            {!setupComplete && (
              <div className="border-t border-slate-700/30 pt-6">
                {!adminExists && userRole === "user" ? (
                  ...
                  <button
                    onClick={() => router.push("/")}
                    className="w-full rounded-lg bg-slate-700/50 px-6 py-3 font-bold text-slate-200 shadow-lg transition-all hover:bg-slate-700/70 uppercase tracking-wide"
                  >
                    Back to Dashboard
                  </button>
                )}
        ...
        <div className="mt-8 rounded-lg border border-slate-700/30 bg-slate-900/20 p-6">
          ...
          <ul className="space-y-2 text-sm text-slate-300">
            <li>✓ If no admin exists yet, you can become the first admin</li>
            <li>✓ Admin users can access the admin dashboard to view all tickets</li>
            <li>✓ Only one first admin can be set up this way</li>
            <li>✓ Additional admins must be added through Firebase Console</li>
          </ul>
        </div>
      </section>
    </main>
